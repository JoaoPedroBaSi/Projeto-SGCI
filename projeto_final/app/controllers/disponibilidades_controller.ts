...
export default class DisponibilidadesController {
    ...
    public async store({request, response}:HttpContext){
        ...
        try {
            ...
            //Garante que os horários serão para datas futuras
            ...
            while (inicioSlot < dataHoraFimTotal) {
                ...
                //Calcula o fim de cada slot, fazendo data_hora_inicio + 30 minutos
                const fimSlot = inicioSlot.plus({ minutes: DURACAO_SLOT_MINUTOS })

                //Verifica se o horário está dentro do período de almoço. Se estiver, vai ser ignorado
                //na geração de slots
                const estaEmHorarioAlmoco = (inicioSlot >= ALMOCO_INICIO && inicioSlot < ALMOCO_FIM)

                if (estaEmHorarioAlmoco) {
                    //Se estiver no almoço, o loop pula para o fim do almoço
                    inicioSlot = ALMOCO_FIM; 
                    continue; //Pula o resto do código e vai para o próximo ciclo
                }
                
                // Se o final do slot ultrapassar o limite de disponibilidade, para.
                if (fimSlot > dataHoraFimTotal) {
                    break 
                }
                
                //Verificação para garantir que os valores são válidos
            ...
            return response.created({ message: 'Disponibilidade e slots criados com sucesso!' })

        } catch (error){
            let message = 'Ocorreu um erro desconhecido.'
            let status = 500
            if (error instanceof Error) {
                if (error.message === 'O horário informado é ultrapassado.') {
                    message = 'Horário ultrapassado. Tente novamente.'
                    status = 400
                } else if (error.message === 'O horário está fora do horário de funcionamento da clínica.') {
                    message = 'Horário fora do funcionamento da clínica. Tente novamente.'
                    status = 400
                } else if (error.message === 'Erro interno: Uma data de slot gerada é inválida.') {
                    message = 'Uma falha interna ocorreu. Tente novamente.'
                    status = 400
                }
            } 
        ...
        public async update({ request, response }: HttpContext) { // Incluindo os argumentos
            ...
            try{
                ...
                await db.transaction(async (trx: TransactionClientContract) => {
                        ...
                        //LEITURA CRÍTICA**: Busca os slots existentes do dia.
