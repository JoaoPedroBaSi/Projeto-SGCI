...
export default class AtendimentosController {
  ...
  public async store ({ request, response }: HttpContext){
    try {
      ...
      const salaReservada = await Reserva.query().where('profissional_id', dados.profissional_id).first()//.andWhere('status', 'APROVADA').andWhere('pagamento_efetuado', true).first()
      if (!salaReservada){
          throw new Error('O profissional escolhido para consulta não tem nenhum sala reservada.')
      }

      await this.atendimentoService.criarAtendimento(dados, salaReservada.salaId, dataHoraInicioLuxon, disponibilidade)
