            ...
            <div v-if="expandirLogin" class="painel-interno">
              <div class="conteudo-padding">
                <form @submit.prevent="fazerLogin">
                  <div class="campo"><label>E-mail</label><input type="email" v-model="loginData.email" required />
                  </div>
                  <div class="campo"><label>Senha</label><input type="password" v-model="loginData.senha" required />
                  </div>
            ...
            <div v-if="expandirCadastro" class="painel-interno">
              <div class="conteudo-padding">
                <form @submit.prevent="fazerCadastroCliente" class="form-interno">
                  ...
                  <div class="campo"><label>Nome Completo</label><input type="text" v-model="formCliente.nome"
                      required /></div>
                  <div class="linha-dupla">
                    <div class="campo metade"><label>Gênero</label><select v-model="formCliente.genero" required>
                        <option value="MASCULINO">Masculino</option>
                        <option value="FEMININO">Feminino</option>
                      </select></div>
                    <div class="campo metade"><label>Nascimento</label><input type="date"
                        v-model="formCliente.dataNascimento" required /></div>
                  </div>
                  <div class="linha-dupla">
                    <div class="campo metade"><label>CPF</label><input type="text" v-model="formCliente.cpf" required />
                    </div>
                    <div class="campo metade"><label>Telefone</label><input type="tel" v-model="formCliente.telefone"
                        required /></div>
                  </div>
                  <h4 class="subtitulo-form">Dados de Acesso</h4>
                  <div class="campo"><label>E-mail</label><input type="email" v-model="formCliente.email" required />
                  </div>
                  <div class="linha-dupla">
                    <div class="campo metade"><label>Senha</label><input type="password" v-model="formCliente.senha"
                        required /></div>
                    <div class="campo metade"><label>Confirmar</label><input type="password"
                        v-model="formCliente.confirmarSenha" required /></div>
                  </div>
