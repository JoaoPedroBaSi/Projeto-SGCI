...
interface ItemEstoque {
    ...
    unidade?: string;
    lote?: string;
    qtySolicitada: number; // Controle local do input
}
...
<style scoped>
.page-container { padding: 30px; font-family: 'Montserrat', sans-serif; color: #4b5563; }

/* HEADER */
.header-section { display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px; }
.page-title { color: #2dd4bf; font-weight: 800; font-size: 1.8rem; margin: 0; }
...
.main-layout {
    display: grid;
    grid-template-columns: 2.5fr 1fr; /* Esquerda maior, Direita menor */
    gap: 25px;
}
...
.panel-header h3 { font-size: 1.1rem; color: #374151; font-weight: 700; margin: 0; }
.btn-outline { border: 1px solid #2dd4bf; color: #2dd4bf; background: none; padding: 6px 15px; border-radius: 6px; font-weight: 600; cursor: pointer; font-size: 0.8rem; }

.search-box { display: flex; margin-bottom: 25px; }
.search-box input { flex: 1; padding: 12px; border: 1px solid #d1d5db; border-radius: 8px 0 0 8px; outline: none; font-family: 'Montserrat'; font-size: 0.9rem; }
.search-box input:focus { border-color: #2dd4bf; }
.search-btn { background: #2dd4bf; border: none; padding: 0 20px; border-radius: 0 8px 8px 0; color: white; cursor: pointer; }

/* Tabela Customizada */
.table-header { display: grid; grid-template-columns: 3fr 1.5fr 1.5fr 1fr; padding-bottom: 10px; border-bottom: 1px solid #f3f4f6; color: #9ca3af; font-size: 0.75rem; font-weight: 700; text-transform: uppercase; margin-bottom: 15px; }
.item-row { display: grid; grid-template-columns: 3fr 1.5fr 1.5fr 1fr; align-items: center; padding: 15px 0; border-bottom: 1px solid #f3f4f6; }

.info { display: flex; flex-direction: column; }
.item-name { font-weight: 700; color: #374151; font-size: 0.95rem; }
.item-desc { font-size: 0.8rem; color: #9ca3af; margin-top: 3px; }

/* Badges de Estoque */
.status-badge { font-size: 0.75rem; font-weight: 700; padding: 4px 10px; border-radius: 12px; display: inline-block; }
.tag-critico { background: #fef2f2; color: #ef4444; }
.tag-baixo { background: #fefce8; color: #eab308; }
.tag-normal { background: #f0fdf4; color: #22c55e; }

/* Contador */
.counter-wrapper { display: flex; align-items: center; gap: 5px; }
.btn-count { width: 28px; height: 28px; border: 1px solid #e5e7eb; background: #f9fafb; border-radius: 4px; cursor: pointer; color: #374151; font-weight: bold; }
.counter-wrapper input { width: 35px; text-align: center; border: none; font-weight: bold; color: #374151; outline: none; }

/* Botão Solicitar */
.btn-request { background: white; border: 1px solid #2dd4bf; color: #2dd4bf; padding: 8px 15px; border-radius: 6px; font-weight: 600; cursor: pointer; transition: all 0.2s; font-size: 0.85rem; }
.btn-request:hover { background: #2dd4bf; color: white; }

/* === PAINEL DIREITO (HISTÓRICO) === */
.history-panel h3 { font-size: 1rem; color: #374151; margin-bottom: 20px; }

.history-item { display: flex; align-items: center; gap: 12px; padding: 12px 0; border-bottom: 1px solid #f3f4f6; }
.icon-box { width: 32px; height: 32px; background: #f3f4f6; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 0.9rem; }
.history-info { flex: 1; display: flex; flex-direction: column; }
.history-name { font-size: 0.85rem; font-weight: 700; color: #374151; }
.history-date { font-size: 0.75rem; color: #9ca3af; }

.history-status { font-size: 0.7rem; font-weight: 800; }
.text-pendente { color: #f59e0b; }
.text-aprovado { color: #10b981; }
.text-rejeitado { color: #ef4444; }

.link-see-all { display: block; text-align: center; margin-top: 20px; color: #2dd4bf; font-weight: 700; font-size: 0.85rem; text-decoration: none; }
.empty-history { text-align: center; color: #9ca3af; font-size: 0.9rem; margin-top: 20px; }
